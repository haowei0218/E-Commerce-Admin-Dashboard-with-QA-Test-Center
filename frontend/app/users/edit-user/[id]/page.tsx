...
import { changePassword, setAdminUserRole, updateMyProfile } from "@/lib/user.api";
...
export default function editUser() {
    ...
    useEffect(() => {
        ...
        const operatorRoleId = operatorUser?.roleId
        if (operatorRoleId === undefined) {
            return
        }
        const roleDisable: boolean[] = ["2", "3", "4", "5", "6"].map((item: string) => operatorRoleId < Number(item))
        ...
        <div className='w-full h-full flex '>
            ...
            <div className='min-w-0 flex-1 flex flex-col'>
                ...
                <div className='bg-gray-50 w-full h-full p-5 overflow-y-auto'>
                    ...
                    {/* 帳號狀態設定 */}
                    <form className=" w-full max-w-6xl mt-10 ml-20 border border-gray-300 rounded-2xl bg-white p-10 ">
                        ...
                        <div className="w-full flex justify-end">
                            <button type="submit" className="w-30 h-9 border flex justify-center items-center border-gray-300 rounded-lg bg-blue-500 font-bold text-white gap-2 hover:bg-blue-700 disabled:bg-gray-400">{statusForm.formState.isLoading ? "變更中..." : "儲存變更"}</button>
                        </div>
                    </form>
                </div>
            </div>
